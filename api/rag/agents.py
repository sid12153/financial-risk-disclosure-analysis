...
#     """
#     Tries to find a JSON object in model output.
#     Keep simple: look for first '{' and last '}'.
#     """
#     start = text.find("{")
#     end = text.rfind("}")
...
# def planner_agent(client: HFInferenceClient, model: str, question: str, doc_id: Optional[str], top_k_default: int) -> Tuple[Plan, float]:
#     prompt = f"""
...
#     res = client.generate(model=model, prompt=prompt, max_new_tokens=200, temperature=0.0)
...
# def verifier_agent(client: HFInferenceClient, model: str, question: str, evidence_chunks: List[Dict], min_score: float) -> Tuple[VerifyResult, float]:
#     # send only the top few snippets to verifier
#     snippets = []
...
# - top score < {min_score}
# - evidence does not mention the topic
...
# def summarizer_agent(client: HFInferenceClient, model: str, question: str, evidence_chunks: List[Dict]) -> Tuple[str, float]:
#     # give it bounded evidence
#     snippets = []
#     for e in evidence_chunks[:5]:
...
# If evidence is insufficient, say: "I can’t answer from the indexed filings."
...
#     res = client.generate(model=model, prompt=prompt, max_new_tokens=350, temperature=0.2)
#     return res.text.strip(), res.latency_ms


# def run_multi_agent(question: str, doc_id: Optional[str], top_k: int, min_score: float,
#                     planner_model: str, verifier_model: str, summary_model: str) -> Dict:
...
#             "plan": plan,
#             "latency": {"planner_ms": planner_ms, "verifier_ms": verifier_ms, "summary_ms": summary_ms},
#             "hits": [],
#             "answer": ""
#         }
...
#         client, verifier_model, question,
